...
Runs = args.runs
idxs_controlled = np.arange(N).tolist()#random.sample( range(0, N),  num_controlled_players)
...
for run in np.arange(0,Runs):
    ...
    for c in range(C):
        ...
        perturbed_capacities = original_capacities
    ...
    pickle.dump([Game_data.Incurred_losses, Game_data.Played_actions, addit_Congestions, Total_occupancies, idxs_controlled], file)
    file.close()
